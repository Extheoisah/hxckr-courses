...
<TestResult title="Test Result" status="not started">
...
<ConceptCard title="Bitcoin Multi-Signature Fundamentals">
  <ListItem title="A multi-signature address requires M-of-N signatures to spend funds, where:">
    M = number of required signatures

    N = total number of possible signers
  </ListItem>

  <ListItem title="Example: In a 2-of-3 setup, any 2 out of 3 designated parties must sign" />
</ConceptCard>

<ConceptCard title="Address Types">
  <ListItem title="P2SH (Pay to Script Hash)">
    Legacy segwit multisig format starting with '2' on testnet

    Embeds script hash in address
  </ListItem>

  <ListItem title="P2WSH (Pay to Witness Script Hash)">
